...
func make_config(t *testing.T, n int, unreliable bool, snapshot bool) *config {
	// 确保只执行一次 CPU 相关的检查和初始化
	ncpu_once.Do(func() {
		// 检查 CPU 核心数，如果只有一个核心可能掩盖锁相关的 bug
		if runtime.NumCPU() < 2 {
...
// applierSnap
// i: 服务器ID  applyCh: 应用消息通道，接收Raft发来的消息
	...
	for m := range applyCh {
		...
		if m.SnapshotValid {
			...
			if (m.CommandIndex+1)%SnapShotInterval == 0 {
				...
				// 编码日志数组
				e.Encode(xlog)
				// 调用Raft的Snapshot方法，传入编码后的字节数组
				rf.Snapshot(m.CommandIndex, w.Bytes())
				// Snapshot在实际应用中的结构类似于：
				// type Snapshot struct {
				//	LastIndex int           // 快照包含的最后一个命令的索引
				//	Commands  []interface{} // 所有已应用的命令
				// }
			}
		} else {
			// Ignore other types of ApplyMsg.
...
func (cfg *config) disconnect(i int) {
	...
	// outgoing ClientEnds - 断开这个服务器向外发送的连接
	for j := 0; j < cfg.n; j++ {
		if cfg.endnames[i] != nil {
			endname := cfg.endnames[i][j]
			cfg.net.Enable(endname, false) // 禁用i->j这条连接
		}
	}
	...
	for j := 0; j < cfg.n; j++ {
		if cfg.endnames[j] != nil {
			endname := cfg.endnames[j][i]
			cfg.net.Enable(endname, false) // 禁用j->i这条连接
		}
	}
}
...
func (cfg *config) onemoretime(cmd interface{}, expectedServers int, retry bool, timelimit float64) int {
	t0 := time.Now()
	starts := 0
	// 设置超时时间，并且确保未完成
	for time.Since(t0).Seconds() < timelimit && cfg.checkFinished() == false {
		// try all the servers, maybe one is the leader.
		index := -1
		for si := 0; si < cfg.n; si++ { // 轮询所有服务器
			starts = (starts + 1) % cfg.n
