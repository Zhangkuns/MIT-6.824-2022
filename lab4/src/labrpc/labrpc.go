...
func MakeService(rcvr interface{}) *Service {
	...
	for m := 0; m < svc.typ.NumMethod(); m++ {
		...
		// 检查是否公开（首字母大写）
		if method.PkgPath != "" {
			fmt.Printf("bad method: %v (not exported, PkgPath: %s)\n", mname, method.PkgPath)
			continue
		}
		// 检查参数数量（接收者 + args + reply = 3）
		if mtype.NumIn() != 3 {
			fmt.Printf("bad method: %v (wrong number of inputs, expected 3, got %d)\n", mname, mtype.NumIn())
			continue
		}
		// 检查 reply 参数是否是指针类型
		if mtype.In(2).Kind() != reflect.Ptr {
			fmt.Printf("bad method: %v (reply parameter is not a pointer, got %v)\n", mname, mtype.In(2).Kind())
			continue
		}
		// 检查返回值数量（应为 0）
		if mtype.NumOut() != 0 {
			fmt.Printf("bad method: %v (wrong number of outputs, expected 1, got %d)\n", mname, mtype.NumOut())
			continue
		}
